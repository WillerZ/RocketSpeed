...
DATA_DIR=${DATA_DIR:-$(mktemp -t -d rocketspeed_XXXX)}
STAT_FILE=${STAT_FILE:-$(mktemp -t -u rocketspeed_test_stats_XXXX)}
...
common_in_mem_args="--db=/dev/shm/rocketspeed \
    --num_levels=6 \
    --key_size=20 \
    --prefix_size=12 \
    --keys_per_prefix=10 \
...
function send_to_ods {
  ...
  curl -s "https://www.intern.facebook.com/intern/agent/ods_set.php?entity=rocketspeed_build$git_br&key=$key&value=$value" \
    --connect-timeout 60
}
...
function send_benchmark_to_ods {
  ...
  send_to_ods rocketspeed.build.$bench_key.qps $QPS
  send_to_ods rocketspeed.build.$bench_key.p50_micros $P50_MICROS
  send_to_ods rocketspeed.build.$bench_key.p75_micros $P75_MICROS
  send_to_ods rocketspeed.build.$bench_key.p99_micros $P99_MICROS
}
