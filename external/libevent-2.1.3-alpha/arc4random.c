...
arc4_seed_proc_sys_kernel_random_uuid(void)
{
	...
	for (bytes = 0; bytes<ADD_ENTROPY; ) {
		fd = ld_evutil_open_closeonexec_("/proc/sys/kernel/random/uuid", O_RDONLY, 0);
		if (fd < 0)
			return -1;
		...
		for (i=nybbles=0; i<n; ++i) {
			if (LD_EVUTIL_ISXDIGIT_(buf[i])) {
				int nyb = ld_evutil_hex_char_to_int_(buf[i]);
				if (nybbles & 1) {
...
arc4_seed_urandom(void)
{
	...
	for (i = 0; filenames[i]; ++i) {
		fd = ld_evutil_open_closeonexec_(filenames[i], O_RDONLY, 0);
		if (fd<0)
			continue;
