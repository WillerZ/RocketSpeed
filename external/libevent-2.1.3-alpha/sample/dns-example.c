...
gai_callback(int err, struct evutil_addrinfo *ai, void *arg)
{
	...
	int i;
	if (err) {
		printf("%s: %s\n", name, ld_evutil_gai_strerror(err));
	}
	...
	for (i=0; ai; ai = ai->ai_next, ++i) {
		...
		if (ai->ai_family == PF_INET) {
			...
			ld_evutil_inet_ntop(AF_INET, &sin->sin_addr, buf,
			    sizeof(buf));
			printf("[%d] %s: %s\n",i,name,buf);
		} else {
			struct sockaddr_in6 *sin6 =
			    (struct sockaddr_in6*)ai->ai_addr;
			ld_evutil_inet_ntop(AF_INET6, &sin6->sin6_addr, buf,
			    sizeof(buf));
			printf("[%d] %s: %s\n",i,name,buf);
		}
	}
...
main(int c, char **v) {
	...
	for (; idx < c; ++idx) {
		if (reverse) {
			struct in_addr addr;
			if (ld_evutil_inet_pton(AF_INET, v[idx], &addr)!=1) {
	...
	fflush(stdout);
	ld_event_base_dispatch(event_base);
	return 0;
}
