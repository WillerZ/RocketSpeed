...
fifo_read(evutil_socket_t fd, short event, void *arg)
{
	...
	if (len <= 0) {
		...
		ld_event_del(ev);
		ld_event_base_loopbreak(ld_event_get_base(ev));
		return;
	}
...
main(int argc, char **argv)
{
	...
	/* Initalize one event */
#ifdef _WIN32
	evfifo = ld_event_new(base, (evutil_socket_t)socket, EV_READ|EV_PERSIST, fifo_read,
                           ld_event_self_cbarg());
#else
	/* catch SIGINT so that event.fifo can be cleaned up */
	signal_int = evsignal_new(base, SIGINT, signal_cb, base);
	ld_event_add(signal_int, NULL);

	evfifo = ld_event_new(base, socket, EV_READ|EV_PERSIST, fifo_read,
                           ld_event_self_cbarg());
#endif

	/* Add it to the active events, without a timeout */
	ld_event_add(evfifo, NULL);
