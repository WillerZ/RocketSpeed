...
http_request_done(struct evhttp_request *req, void *ctx)
{
	...
	while ((nread = ld_evbuffer_remove(evhttp_request_get_input_buffer(req),
...
main(int argc, char **argv)
{
	...
	if (strcasecmp(scheme, "http") == 0) {
		bev = ld_bufferevent_socket_new(base, -1, BEV_OPT_CLOSE_ON_FREE);
	} else {
	...
	// asynchronous though.
	evcon = evhttp_connection_base_ld_bufferld_event_new(base, NULL, bev,
		host, port);
	if (evcon == NULL) {
		fprintf(stderr, "evhttp_connection_base_ld_bufferld_event_new() failed\n");
		return 1;
	}
