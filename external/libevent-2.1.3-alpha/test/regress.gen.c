...
void ld_event_warn(const char *fmt, ...);
void ld_event_warnx(const char *fmt, ...);
    ...
    const struct run* value)
{
...

  {
    ...
    if (run_unmarshal(msg->run_data[off], tmp) == -1) {
      ld_event_warnx("%s: run_unmarshal", __func__);
      had_error = 1;
      goto done;
    }
    done:if (tmp != NULL)
...
{
  ev_uint32_t tag;
  while (ld_evbuffer_get_length(evbuf) > 0) {
    ...
    switch (tag) {
      ...
      case MSG_FROM_NAME:
        ...
        if (evtag_unmarshal_string(evbuf, MSG_FROM_NAME, &tmp->from_name_data) == -1) {
          ld_event_warnx("%s: failed to unmarshal from_name", __func__);
          return (-1);
        }
        tmp->from_name_set = 1;
        break;
      ...
      case MSG_TO_NAME:
        ...
        if (evtag_unmarshal_string(evbuf, MSG_TO_NAME, &tmp->to_name_data) == -1) {
          ld_event_warnx("%s: failed to unmarshal to_name", __func__);
          return (-1);
        }
        tmp->to_name_set = 1;
        break;
      ...
      case MSG_ATTACK:
        ...
        if (evtag_unmarshal_kill(evbuf, MSG_ATTACK, tmp->attack_data) == -1) {
          ld_event_warnx("%s: failed to unmarshal attack", __func__);
          return (-1);
        }
        tmp->attack_set = 1;
        break;
      ...
      case MSG_RUN:
        ...
        if (evtag_unmarshal_run(evbuf, MSG_RUN, tmp->run_data[tmp->run_length]) == -1) {
          ld_event_warnx("%s: failed to unmarshal run", __func__);
          return (-1);
        }
        ++tmp->run_length;
        tmp->run_set = 1;
...
{
  struct evbuffer *buf_ = ld_evbuffer_new();
  assert(buf_ != NULL);
  msg_marshal(buf_, msg);
  evtag_marshal_buffer(evbuf, tag, buf_);
   ld_evbuffer_free(buf_);
}
    ...
    switch (tag) {
      ...
      case KILL_WEAPON:
        ...
        if (evtag_unmarshal_string(evbuf, KILL_WEAPON, &tmp->weapon_data) == -1) {
          ld_event_warnx("%s: failed to unmarshal weapon", __func__);
          return (-1);
        }
        tmp->weapon_set = 1;
        break;
      ...
      case KILL_ACTION:
        ...
        if (evtag_unmarshal_string(evbuf, KILL_ACTION, &tmp->action_data) == -1) {
          ld_event_warnx("%s: failed to unmarshal action", __func__);
          return (-1);
        }
        tmp->action_set = 1;
        break;
      ...
      case KILL_HOW_OFTEN:
        ...
        if (evtag_unmarshal_int(evbuf, KILL_HOW_OFTEN, &tmp->how_often_data[tmp->how_often_length]) == -1) {
          ld_event_warnx("%s: failed to unmarshal how_often", __func__);
          return (-1);
        }
    ...
    const char * value)
{
...

  {
    ...
    if (msg->notes_data[off] == NULL) {
      ld_event_warnx("%s: strdup", __func__);
      return (-1);
    }
  }
  return (0);
    ...
    switch (tag) {
      ...
      case RUN_HOW:
        ...
        if (evtag_unmarshal_string(evbuf, RUN_HOW, &tmp->how_data) == -1) {
          ld_event_warnx("%s: failed to unmarshal how", __func__);
          return (-1);
        }
        tmp->how_set = 1;
        break;
      ...
      case RUN_SOME_BYTES:
        ...
        if (evtag_payload_length(evbuf, &tmp->some_bytes_length) == -1)
          return (-1);
        if (tmp->some_bytes_length > ld_evbuffer_get_length(evbuf))
          return (-1);
        ...
        if (evtag_unmarshal_fixed(evbuf, RUN_SOME_BYTES, tmp->some_bytes_data, tmp->some_bytes_length) == -1) {
          ld_event_warnx("%s: failed to unmarshal some_bytes", __func__);
          return (-1);
        }
      ...
      case RUN_FIXED_BYTES:
        ...
        if (evtag_unmarshal_fixed(evbuf, RUN_FIXED_BYTES, tmp->fixed_bytes_data, (24)) == -1) {
          ld_event_warnx("%s: failed to unmarshal fixed_bytes", __func__);
          return (-1);
        }
      ...
      case RUN_NOTES:
        ...
        if (evtag_unmarshal_string(evbuf, RUN_NOTES, &tmp->notes_data[tmp->notes_length]) == -1) {
          ld_event_warnx("%s: failed to unmarshal notes", __func__);
          return (-1);
        }
        ++tmp->notes_length;
        tmp->notes_set = 1;
      ...
      case RUN_LARGE_NUMBER:
        ...
        if (evtag_unmarshal_int64(evbuf, RUN_LARGE_NUMBER, &tmp->large_number_data) == -1) {
          ld_event_warnx("%s: failed to unmarshal large_number", __func__);
          return (-1);
        }
      ...
      case RUN_OTHER_NUMBERS:
        ...
        if (evtag_unmarshal_int(evbuf, RUN_OTHER_NUMBERS, &tmp->other_numbers_data[tmp->other_numbers_length]) == -1) {
          ld_event_warnx("%s: failed to unmarshal other_numbers", __func__);
          return (-1);
        }
