...
class CacheTest {
 public:
  ...
  CacheTest() :
      ...
      cache2_(NewLRUCache(kCacheSize2)) {
    current_ = this;
  }
...
TEST(CacheTest, UsageTest) {
  ...
  ASSERT_GE(kCapacity, cache->GetUsage());
  ASSERT_LT(kCapacity * 0.95, cache->GetUsage());
}

TEST(CacheTest, PinnedUsageTest) {
  // cache is shared_ptr and will be automatically cleaned up.
  const uint64_t kCapacity = 100000;
  auto cache = NewLRUCache(kCapacity);
...
TEST(CacheTest, SetCapacity) {
  ...
  std::shared_ptr<Cache> cache = NewLRUCache(5);
...
TEST(CacheTest, OverCapacity) {
  ...
  std::shared_ptr<Cache> cache = NewLRUCache(n);
...
TEST(CacheTest, ApplyToAllCacheEntiresTest) {
  ...
  cache_->ApplyToAllCacheEntries(callback);
