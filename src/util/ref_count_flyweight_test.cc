...
TEST(RefCountFlyweightTest, FlyweightsCanBeDestroyedBeforeOrAfterFactory) {
...

  {
    ...
    RefCountFlyweight<InstanceCounter> sharedDestroyedAfterFactory;
    {
      ...
      ASSERT_TRUE("All good when flyweight is destroyed before factory");
    }
    ...
    ASSERT_EQ(sharedDestroyedAfterFactory.Get().payload_,
              InstanceCounter::PAYLOAD);
    ASSERT_TRUE("All good when non-empty flyweight factory is destroyed");
  }
  ASSERT_EQ(instanceCountUniqueAfter, 0);
  ASSERT_EQ(instanceCountShared, 0);
  ASSERT_TRUE("All good when flyweight is destroyed after factory");
}
