...
class IDChecker : public ThreadLocalPtr {
 public:
  // This is kind-of hacky. The rocketspeed library uses two static
  // variables to cache the following:
  // (1) A thread local GUID generator in messages.cc
  // (2) A thread local thread name in base-env.cc
  // For this reason, the startid of ThreadLocals start at 2.
  static uint32_t PeekId() { return Instance()->PeekId() - 2; }
};
